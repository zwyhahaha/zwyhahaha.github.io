...
For each layer $l$, the weight is ${\mathbf{w}}_l = (w_{l 1}, ..., w_{l 
e}) > 0$, the number of physical experts (replicas) is $c$. The decision variable is the number of replicas for each logical expert, ${\mathbf{r}}_l = (r_{l 1}, ..., r_{l e})$. The optimization problem is,
